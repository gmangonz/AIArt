...
def get_xy_gradients(sigma, resolution=32):
    ...
    """ Will create random x and y values (between -1 & 1) for gradient vectors """
...
def make_displacement_map(frame, x_gradients, y_gradients, amplitude, shape=(512, 512)):
    ...
    # Rotate gradients based on frame value to create faux 3D noise that I can control
...
def get_displacement_map(frame_value, amplitude_factor, resolution_factor, shape=(512, 512)):
    ...
    """
    Using 3 different audio signals, we get frame_value, amplitude_factor, resolution_factor

    Inputs:
      frame_value:       between 0 and 1, will be mapped to 0-360 = will rotate the gradient vectors to create faux 3D noise
    ...
    Returns:
      ...
      displacement_map: displacement map for tf.image.dense_image_warp, (shape[0], shape[1], 2)

      noise_image: noise image to input into StyleGAN2, (shape[0], shape[1], 3)
